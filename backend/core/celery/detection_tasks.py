...
# Import XAI function
from .explainable_ai import run_explainable_ai
...
def base64_to_image(base64_str: str) -> np.ndarray:
    ...
    # FIX: removed duplicate decode — only one nparr/imdecode call needed
                ...
                "results": [
                    {
                        ...
                        "fake_prob": round(fake_prob, 4),
                        "real_prob": round(real_prob, 4),
                        "frame_data": frame_data,   # explainable_ai.py reads "frame_data", not "original_frame_data"
                    }
                ]
            }
            run_explainable_ai.delay(task_id, xai_payload)
            logger.info(f"[GenD Inference] XAI task dispatched for task_id={task_id}, frame={frame_index}")
